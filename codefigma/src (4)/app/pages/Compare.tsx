...
export default function Compare() {
  ...
  const addCourse = (id: string) => {
    if (selectedIds.length >= MAX_COMPARE) { toast.warning(`Chỉ so sánh tối đa ${MAX_COMPARE} khóa học`); return; }
    if (selectedIds.includes(id)) { toast.info('Khóa học đã được chọn'); return; }
  ...
  const compareRows: { label: string; key: keyof typeof courses[0]; render?: (v: any) => React.ReactNode }[] = [
    { label: 'Giá', key: 'price', render: (v: number) => <span className="font-bold text-blue-600">{v.toLocaleString()}K₫</span> },
    { label: 'Giá gốc', key: 'originalPrice', render: (v: number) => <span className="line-through text-gray-400">{v.toLocaleString()}K₫</span> },
                  ...
                  {selectedCourses.map(course => (
                    <th key={course.id} className="min-w-[220px] px-3 pb-4 align-top">
                      <div className="bg-white rounded-xl border border-gray-200 p-4 relative shadow-sm">
                        ...
                        <button onClick={() => { addToCart(course); toast.success('Đã thêm vào giỏ hàng!'); }}
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden">
              ...
              <div className="overflow-y-auto max-h-[calc(80vh-80px)] p-4">
                <div className="grid sm:grid-cols-2 gap-3">
                  {courses.filter(c => !selectedIds.includes(c.id)).map(course => (
                    <button key={course.id} onClick={() => addCourse(course.id)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs font-bold text-blue-600 mt-1">{course.price.toLocaleString()}K₫</p>
                      </div>
                    </button>
                  ))}
                </div>
        ...
        {selectedCourses.length > 0 && selectedCourses.length < MAX_COMPARE && !showPicker && (
          <div className="mt-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {courses.filter(c => !selectedIds.includes(c.id)).slice(0, 4).map(course => (
                <div key={course.id} className="bg-white rounded-xl border border-gray-200 p-3 hover:border-blue-300 transition">
                  ...
                  <p className="text-xs text-gray-500 mb-2">{course.price.toLocaleString()}K₫</p>
