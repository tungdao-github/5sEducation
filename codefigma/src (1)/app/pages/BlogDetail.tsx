...
const mockComments = [
  { id: 'c1', author: 'Nguyễn Minh Khoa', avatar: 'K', date: '02/04/2026', text: 'Bài viết rất hay và chi tiết! Mình đã áp dụng một số nguyên tắc này vào dự án và thấy hiệu quả rõ rệt.', likes: 12, postId: 'b1' },
  { id: 'c2', author: 'Trần Thị Lan', avatar: 'L', date: '30/03/2026', text: 'Cảm ơn tác giả đã chia sẻ! Mình đang học khóa Gestalt trên EduCourse và thấy bài viết này bổ sung thêm rất nhiều kiến thức.', likes: 8, postId: 'b1' },
  { id: 'c3', author: 'Lê Văn Dũng', avatar: 'D', date: '28/03/2026', text: 'Phần về Closure rất thú vị. Mình chưa để ý đến chi tiết này trước đây!', likes: 5, postId: 'b1' },
];

function CommentsSection({ postId }: { postId: string }) {
  const { user, isAuthenticated, openAuthModal } = useAuth();
  const [comments, setComments] = useState(mockComments.filter(c => c.postId === postId || true).slice(0, 3));
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mt-2 pl-2">
                <button onClick={() => handleLike(comment.id)}
                  className={`flex items-center gap-1.5 text-xs transition ${likedIds.includes(comment.id) ? 'text-blue-600 font-medium' : 'text-gray-400 hover:text-blue-600'}`}>
                  <ThumbsUp className={`size-3.5 ${likedIds.includes(comment.id) ? 'fill-blue-600' : ''}`} />
                  {comment.likes} thích
                </button>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <article className="lg:col-span-2">
            ...
            <div className="mt-6 p-5 bg-blue-50 rounded-xl flex items-center justify-between gap-4">
              ...
              <button
                onClick={() => navigator.clipboard.writeText(window.location.href).then(() => toast.success('Đã sao chép link!'))}
