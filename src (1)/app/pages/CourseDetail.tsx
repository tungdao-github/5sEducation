...
import { Clock, BarChart3, Users, Star, ShoppingCart, Check, PlayCircle, Award, BookOpen, ChevronDown, ThumbsUp, MessageSquare } from 'lucide-react';
...
export default function CourseDetail() {
  ...
  const { getReviewsByCourse, addReview, hasUserReviewed, getCourseAverageRating, markHelpful } = useReviews();
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg overflow-hidden shadow-xl sticky top-20">
                ...
                <div className="p-6">
                  ...
                  {/* Quick-access learning link */}
                  <Link
                    to={`/learn/${course.id}`}
                    className="block w-full py-2.5 rounded-lg font-semibold text-center border border-blue-300 text-blue-600 hover:bg-blue-50 transition-colors mt-2 text-sm"
                  >
                    🎓 Vào học ngay (Demo)
                  </Link>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-2xl font-bold text-gray-900">{avgRating > 0 ? avgRating : course.rating}</span>
                      </div>
                      <span className="text-gray-600">({reviewCount > 0 ? reviewCount : course.students} đánh giá)</span>
                    </div>
                  </div>
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {review.userLevel && (
                                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                                  review.userLevel === 'Platinum' ? 'bg-purple-100 text-purple-800' :
                                  review.userLevel === 'Gold' ? 'bg-yellow-100 text-yellow-800' :
                                  review.userLevel === 'Silver' ? 'bg-gray-100 text-gray-800' :
