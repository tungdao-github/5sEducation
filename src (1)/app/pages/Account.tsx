...
import {
  ...
  MapPin, Plus, Trash2, Gift, Award, Zap, Shield, Copy, Check,
...
export default function Account() {
  ...
  const [refundModal, setRefundModal] = useState<{ show: boolean; orderId: string | null }>({ show: false, orderId: null });
  ...
  const handleRefundRequest = () => {
    ...
    toast.success(`Yêu cầu hoàn tiền cho đơn hàng ${refundModal.orderId} đã được gửi! Chúng tôi sẽ xem xét trong 3-5 ngày làm việc.`);
              ...
              {activeTab === 'password' && (
                <div className="space-y-8">
                  ...
                  <div className="border-t border-gray-200 pt-8">
                    <TwoFactorSettings user={user} updateUser={updateUser} />
                  </div>
                </div>
              )}
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-red-600 text-white py-2.5 rounded-xl font-semibold hover:bg-red-700 transition-colors text-sm disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
...
function AddressesTab() {
  const [addresses, setAddresses] = useState([
    { id: 1, name: 'Nguyễn Văn A', phone: '0123456789', address: '123 Đường Số 1, Quận 1, TP.HCM' },
    { id: 2, name: 'Nguyễn Văn B', phone: '0987654321', address: '456 Đường Số 2, Quận 2, TP.HCM' },
  ]);
        ...
        <div className="space-y-4">
          {addresses.map(addr => (
            <div key={addr.id} className="border border-gray-200 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between bg-gray-50 px-4 py-3 gap-3">
                <div>
                  <p className="font-mono font-semibold text-gray-900 text-sm">{addr.name}</p>
              ...
              <div className="p-4 space-y-2">
                <p className="text-sm text-gray-800 truncate">{addr.address}</p>
              </div>
            </div>
          ))}
          <button onClick={() => setIsAdding(true)}
      ...
      {isAdding && (
        <div className="mt-5">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Họ và tên</label>
              <input value={newAddress.name} onChange={e => setNewAddress(f => ({ ...f, name: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Số điện thoại</label>
              <input value={newAddress.phone} onChange={e => setNewAddress(f => ({ ...f, phone: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="sm:col-span-2">
...
function TwoFactorSettings({ user, updateUser }: { user: any; updateUser: (data: any) => void }) {
