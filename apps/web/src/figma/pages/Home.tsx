...
function FlashSaleCountdown({ courses }: { courses: Course[] }) {
  ...
  const flashCourses = courses
    ...
    .slice(0, 4)
    .map((c) => ({ ...c, flashPrice: c.price }));

  if (flashCourses.length === 0) return null;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {[
                ...
                <div key={unit.label} className="flex items-center gap-1.5">
                  <div className="bg-white/20 backdrop-blur rounded-lg px-3 py-2 text-center min-w-[50px]">
                    <div className="text-white font-mono text-xl font-bold leading-none">
                      {unit.value}
                    </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {flashCourses.map((course) => (
            <Link key={course.id} to={`/course/${course.slug ?? course.id}`} className="group">
              <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute top-2 left-2 bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full animate-pulse">
                    -
                    {Math.round(
                      ((course.originalPrice ?? course.price) - course.flashPrice) /
                ...
                <div className="p-3">
                  <p className="font-semibold text-gray-900 text-xs line-clamp-2 mb-2">
                    {course.title}
                  </p>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-red-600 text-sm">
                      {formatPriceCompact(course.flashPrice)}
                    </span>
                    <span className="text-gray-400 text-xs line-through">
                      {formatPriceCompact(course.originalPrice ?? course.price)}
                    </span>
                  </div>
        ...
        <div className="text-center mt-6">
          <Link
            to="/search"
            className="inline-flex items-center gap-2 bg-white text-red-600 font-semibold px-6 py-2.5 rounded-xl hover:bg-red-50 transition text-sm"
          >
...
const testimonials = [
  ...
  },
  {
    ...
    avatar: "H",
    rating: 5,
    text: "Khóa Microcopy là một game-changer! Tôi đã tăng được tỉ lệ chuyển đổi của khách hàng lên 40% chỉ bằng cách viết lại button và form labels.",
  },
  {
    name: "Phạm Minh Đức",
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {instructors.map((inst) => (
            <div key={inst.name} className="text-center group cursor-pointer">
              <div
                className={`size-20 bg-gradient-to-br ${inst.color} rounded-2xl flex items-center justify-center text-white text-3xl font-bold mx-auto mb-4 group-hover:scale-105 transition-transform shadow-lg`}
              >
                {inst.avatar}
              </div>
              ...
              <div className="flex items-center justify-center gap-3 text-xs text-gray-600">
                <span className="flex items-center gap-1">
                  <BookOpen className="size-3" />{inst.courses} KH
                </span>
                <span className="flex items-center gap-1">
                  <Users className="size-3" />{inst.students.toLocaleString()}
                </span>
                <span className="flex items-center gap-1">
                  <Star className="size-3 text-yellow-400 fill-yellow-400" />{inst.rating}
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {learningPaths.map((path) => (
            <div
              key={path.title}
              className={`border-2 ${path.color} rounded-2xl p-6 hover:shadow-md transition cursor-pointer`}
            >
              {path.badge && (
                <span className="inline-block text-xs font-bold bg-blue-600 text-white px-2.5 py-0.5 rounded-full mb-3">
                  {path.badge}
                </span>
              )}
              ...
              <Link
                to="/search"
                className="block w-full text-center bg-white border border-current text-blue-600 py-2.5 rounded-xl text-sm font-medium hover:bg-blue-600 hover:text-white transition"
              >
                Bắt đầu lộ trình →
              </Link>
            </div>
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <form onSubmit={handleSearch} className="flex gap-3 mb-6">
                ...
                <button
                  type="submit"
                  className="bg-yellow-400 hover:bg-yellow-300 text-yellow-900 font-bold px-6 py-4 rounded-xl transition whitespace-nowrap"
                >
                  Tìm ngay
                </button>
              </form>
                ...
                <div className="bg-white/10 backdrop-blur rounded-2xl p-6 space-y-4">
                  {featuredCourses.map((course) => (
                    <Link
                      ...
                      className="flex items-center gap-3 hover:bg-white/10 rounded-xl p-2 transition"
                    >
                      <img
                        src={course.image}
                        alt={course.title}
      ...
      <section className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Bắt đầu hành trình học tập ngay hôm nay</h2>
          <p className="text-xl text-blue-100 mb-8">Tham gia cùng 50,000+ học viên đang phát triển sự nghiệp UX/UI Design</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              to="/search"
              className="bg-white text-blue-600 px-8 py-3.5 rounded-xl font-bold hover:bg-blue-50 transition text-sm"
            >
              Khám phá khóa học
            </Link>
            <Link
              to="/compare"
              className="border-2 border-white text-white px-8 py-3.5 rounded-xl font-bold hover:bg-white/10 transition text-sm"
            >
              So sánh khóa học
            </Link>
          </div>
