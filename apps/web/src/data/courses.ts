...
export type CourseCategoryKey = (typeof COURSE_CATEGORY_KEYS)[number]["slug"];

export type CourseCategoryOption = {
  value: CourseCategoryKey;
  label: string;
};

export function getCourseCategoryOptions(locale: AppLocale = "en"): CourseCategoryOption[] {
  return COURSE_CATEGORY_KEYS.map((item) => ({
    value: item.slug,
    label: pickLocaleText(locale, item.en, item.vi, item.es, item.fr),
  }));
}

export function getCourseCategories(locale: AppLocale = "en"): string[] {
  return getCourseCategoryOptions(locale).map((item) => item.label);
}

export const categories = getCourseCategories("vi");
