...
export type SupportMessageSummaryDto = {
  id: number;
  message: string;
  status: string;
  createdAt: string;
...
export type SupportReplySummaryDto = {
  id: number;
...
export type VideoUploadSessionDto = {
  uploadUrl: string;
  videoUid: string;
  playerUrl: string;
};

export type LocalVideoUploadResponseDto = {
  videoUrl: string;
  fileName: string;
};
...
export async function fetchBlogPostDetail(slug: string, locale?: string) {
  const qs = locale ? `?locale=${encodeURIComponent(locale)}` : "";
  return fetchJson<BlogPostDetailDto>(`/api/blog/posts/${slug}${qs}`);
}
...
export function mapBlogPostList(dto: BlogPostListDto, locale: AppLocale = "en"): BlogPost {
  const dateValue = dto.publishedAt ?? dto.createdAt;
  const category = dto.tags?.[0] ?? pickLocaleText(locale, "News", "Tin tức", "Noticias", "Nouvelles");
  return {
    id: String(dto.id),
    title: dto.title,
    slug: dto.slug,
...
export function mapBlogPostDetail(dto: BlogPostDetailDto, locale: AppLocale = "en"): BlogPost {
  const dateValue = dto.publishedAt ?? dto.createdAt;
  const category = dto.tags?.[0] ?? pickLocaleText(locale, "News", "Tin tức", "Noticias", "Nouvelles");
  return {
    id: String(dto.id),
...
export async function fetchCurrentUser() {
  return fetchJsonWithAuth<CurrentUserDto>("/api/auth/me");
}

export async function confirmEmail(userId: string, token: string) {
  return fetchJson<{ message?: string; title?: string }>("/api/auth/confirm-email", {
    method: "POST",
...
export async function forgotPassword(email: string) {
  ...
  return fetchJson<AuthResponseDto>("/api/auth/forgot-password", {
    method: "POST",
...
export async function resetPassword(email: string, token: string, newPassword: string) {
  return fetchJson<AuthResponseDto>("/api/auth/reset-password", {
    method: "POST",
...
export async function fetchSearchSuggestions(query: string, signal?: AbortSignal) {
  return fetchJson<SearchSuggestionDto[]>(
...
export async function fetchSiteSettings(keys: string[]) {
  return fetchJson<SiteSettingDto[]>(
    `/api/settings?keys=${keys.map((key) => encodeURIComponent(key)).join(",")}`,
  );
}

export async function trackCourseView(courseId: number) {
  return fetchJsonWithAuth<void>("/api/history/views", {
    method: "POST",
    body: JSON.stringify({ courseId }),
  });
}

export async function addCourseToCart(courseId: number, quantity: number = 1) {
  return fetchJsonWithAuth<void>("/api/cart", {
    method: "POST",
...
export async function enrollInCourse(courseId: number) {
  return fetchJsonWithAuth<void>("/api/enrollments", {
    method: "POST",
    body: JSON.stringify({ courseId }),
  });
}
...
export async function addCourseToWishlist(courseId: number) {
  return fetchJsonWithAuth<void>("/api/wishlist", {
    method: "POST",
    body: JSON.stringify({ courseId }),
  });
}
...
export async function fetchSupportReplies(messageId: number) {
  return fetchJsonWithAuth<SupportReplySummaryDto[]>(
    `/api/support/messages/${messageId}/replies`,
  );
}
...
export async function createSupportReply(messageId: number, message: string) {
  return fetchJsonWithAuth<SupportReplySummaryDto>(
    `/api/support/messages/${messageId}/replies`,
    {
      method: "POST",
...
export async function deleteLesson(id: number | string) {
  return fetchJsonWithAuth<void>(`/api/lessons/${id}`, { method: "DELETE" });
}

export async function createVideoUploadSession(courseId: number) {
  return fetchJsonWithAuth<VideoUploadSessionDto>("/api/uploads/video", {
    method: "POST",
    body: JSON.stringify({ courseId }),
  });
}

export async function uploadLocalVideo(courseId: number | null | undefined, file: File) {
  const formData = new FormData();
  if (courseId !== null && courseId !== undefined) {
    formData.append("courseId", String(courseId));
  }
  formData.append("file", file);
  return fetchJsonWithAuth<LocalVideoUploadResponseDto>("/api/uploads/video/local", {
    method: "POST",
    body: formData,
  });
}

export async function fetchLearningPathBySlug(slug: string) {
  return fetchJson<{
    id: number;
    title: string;
    slug: string;
