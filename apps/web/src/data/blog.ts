...
export function getBlogCategories(locale: AppLocale = "en"): string[] {
  return BLOG_CATEGORY_KEYS.map((item) => pickLocaleText(locale, item.en, item.vi, item.es, item.fr));
}

export function getBlogCategoryOptions(locale: AppLocale = "en") {
  return BLOG_CATEGORY_KEYS.map((item) => ({
    value: item.slug,
...
const BLOG_CATEGORY_LOOKUP: Record<string, { en: string; vi: string; es: string; fr: string }> = {
  "Thiết kế UX/UI": {
    en: "UX/UI Design",
    vi: "Thiết kế UX/UI",
    es: "Diseño UX/UI",
...
export function getBlogCategoryLabel(locale: AppLocale, category: string) {
  const item = BLOG_CATEGORY_LOOKUP[category];
  if (!item) return category;
  return pickLocaleText(locale, item.en, item.vi, item.es, item.fr);
}

export function getLocalizedBlogPosts(locale: AppLocale = "en"): BlogPost[] {
  return blogPosts.map((post) => ({
    ...post,
    title: pickLocaleText(locale, post.titleEn, post.title, post.titleEn, post.titleEn),
    excerpt: pickLocaleText(locale, post.excerptEn, post.excerpt, post.excerptEn, post.excerptEn),
    content: pickLocaleText(locale, post.contentEn, post.content, post.contentEn, post.contentEn),
    category: getBlogCategoryLabel(locale, post.category),
  }));
}

export const blogCategories = getBlogCategories("vi");
